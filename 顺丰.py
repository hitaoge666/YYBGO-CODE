...
# ==========================================================
# 功能说明：顺丰速运+ 中秋博饼集礼盒 code 版 (YYB-Go 动态适配版)
# 机制：本地 code 服务获取微信 code → UCMP appOnLogin 换 sessionId
...
# ---- 自动读取多账号配置（支持 YYB_GO、CODE_SERVER 逗号/换行符分割） ----
SERVERS = []
env_servers = os.getenv("YYB_GO") or os.getenv("CODE_SERVER") or os.getenv("SF_SERVERS")
if env_servers:
    for line in env_servers.replace(",", "\n").splitlines():
        line = line.strip()
        if line:
            SERVERS.append(line)
if not SERVERS:
...
def get_code(server_entry: str) -> str | None:
    """从 YYB-Go 或本地服务获取微信 code (适配 OPPO / 小蚕 逻辑)"""
    ...
    if "@" in server_entry:
        ...
        print(f"🔐 [授权] 请求 YYB-Go 动态获取 code (标识: {mask(ref)}): {url}")
        try:
